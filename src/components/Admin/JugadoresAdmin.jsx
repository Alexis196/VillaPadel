...
const blankForm = () => ({ name: '', catId: 'cat-8va', sexo: 'M' })
...
export default function JugadoresAdmin() {
  ...
  function handleFilterSexo(val) { setFilterSexo(v => v === val ? 'all' : val); setPage(1) }
  ...
  function startEdit(p) {
    ...
    setForm({ name: p.name, catId: cat.id, sexo: p.sexo || 'M' })
    setEditId(p.id)
  }
  ...
  async function handleSave() {
    ...
    setSaving(true)
    if (editId) {
      await updateJugador(editId, { name: form.name.trim(), categoryName: cat.name, categoryColor: cat.color, categoriaValor: cat.valor, sexo: form.sexo })
      setPlayers(prev => prev.map(p => p.id === editId ? { ...p, name: form.name.trim(), categoryName: cat.name, categoryColor: cat.color, categoriaValor: cat.valor, sexo: form.sexo } : p))
    } else {
      await addJugador({ name: form.name.trim(), categoryName: cat.name, categoryColor: cat.color, categoriaValor: cat.valor, sexo: form.sexo })
      await load()
    }
    setSaving(false)
    setForm(blankForm())
    ...
    <div>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end', flexWrap: 'wrap' }}>
          ...
          <div style={{ minWidth: 120 }}>
            ...
            <div style={{ display: 'flex', background: '#0f0f13', border: '1px solid #2a2a38', borderRadius: 6, padding: 3, gap: 3 }}>
              {[{ v: 'M', label: 'Masculino', col: '#3b82f6' }, { v: 'F', label: 'Femenino', col: '#ec4899' }].map(({ v, label, col }) => (
                <button key={v} type="button" onClick={() => setForm(p => ({ ...p, sexo: v }))}
                  style={{ flex: 1, padding: '4px 0', borderRadius: 4, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 600, transition: 'all 0.15s',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '6px 10px', background: `${cat.color}15`, border: `1px solid ${cat.color}30`, borderRadius: 6 }}>
            ...
            <span style={{ color: cat.color, fontSize: 12, fontWeight: 600 }}>Nivel {cat.valor}</span>
          </div>
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={handleSave} disabled={saving || !form.name.trim()} style={{ background: '#f97316', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 18px', fontSize: 13, fontWeight: 600, cursor: saving || !form.name.trim() ? 'not-allowed' : 'pointer', opacity: saving || !form.name.trim() ? 0.6 : 1 }}>
              {saving ? '...' : editId ? 'Guardar cambios' : '+ Agregar'}
            </button>
          </div>
        </div>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 10, marginBottom: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', gap: 3, flexWrap: 'wrap' }}>
            {[{ id: 'all', label: 'Todas' }, ...CAT_OPTIONS.slice().reverse().map(c => ({ id: c.id, label: c.name.split(' ')[0] }))].map(c => (
              <button key={c.id} onClick={() => handleFilterCat(c.id)} style={{ padding: '4px 9px', borderRadius: 6, border: '1px solid', fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', background: filterCat === c.id ? '#f97316' : 'transparent', color: filterCat === c.id ? '#fff' : '#9999b0', borderColor: filterCat === c.id ? '#f97316' : '#2a2a38' }}>{c.label}</button>
            ))}
          </div>
          <div style={{ width: 1, height: 18, background: '#2a2a38' }} />
          {/* Sexo filter */}
          {[{ v: 'M', label: 'M' }, { v: 'F', label: 'F' }].map(({ v, label }) => (
            <button key={v} onClick={() => handleFilterSexo(v)} style={{ padding: '4px 10px', borderRadius: 6, border: '1px solid', fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', background: filterSexo === v ? (v === 'M' ? '#3b82f6' : '#ec4899') : 'transparent', color: filterSexo === v ? '#fff' : '#9999b0', borderColor: filterSexo === v ? (v === 'M' ? '#3b82f6' : '#ec4899') : '#2a2a38' }}>{label}</button>
          ))}
          {/* Page size */}
      ...
      {loading ? <Spinner /> : filtered.length === 0 ? (
        ...
        <div style={{ background: '#13131a', borderRadius: 12, border: '1px solid #2a2a38', overflow: 'hidden' }}>
          <div style={{ display: 'grid', gridTemplateColumns: '48px 1fr 44px 200px 44px 80px', padding: '0 16px', height: 40, borderBottom: '1px solid #2a2a38', background: '#0f0f13', alignItems: 'center', gap: 10 }}>
            {['#', 'Jugador', 'Sexo', 'Categoría', '↑', ''].map(h => (
          ...
          {paginated.map((p, idx) => {
            ...
            return (
              <div key={p.id} style={{
                display: 'grid', gridTemplateColumns: '48px 1fr 44px 200px 44px 80px',
              ...
              >
                ...
                <div>
                  <span style={{
                    ...
                    background: p.sexo === 'F' ? 'rgba(236,72,153,0.15)' : 'rgba(59,130,246,0.15)',
                    color: p.sexo === 'F' ? '#ec4899' : '#3b82f6',
                    border: `1px solid ${p.sexo === 'F' ? 'rgba(236,72,153,0.3)' : 'rgba(59,130,246,0.3)'}`,
                  }}>{p.sexo === 'F' ? 'F' : 'M'}</span>
                </div>
                <div>
